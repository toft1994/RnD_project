...
--        others - reserved
-- 0x10 : Data signal of numOfInNeurons
--        bit 15~0 - numOfInNeurons[15:0] (Read/Write)
--        others   - reserved
-- 0x14 : reserved
-- 0x18 : Data signal of numOfOutNeurons
--        bit 15~0 - numOfOutNeurons[15:0] (Read/Write)
--        others   - reserved
-- 0x1c : reserved
-- 0x20 : Data signal of activation
...
architecture behave of nnlayer_control_s_axi is
    ...
    constant ADDR_NUMOFINNEURONS_DATA_0  : INTEGER := 16#10#;
    constant ADDR_NUMOFINNEURONS_CTRL    : INTEGER := 16#14#;
    constant ADDR_NUMOFOUTNEURONS_DATA_0 : INTEGER := 16#18#;
    constant ADDR_NUMOFOUTNEURONS_CTRL   : INTEGER := 16#1c#;
    constant ADDR_ACTIVATION_DATA_0      : INTEGER := 16#20#;
    constant ADDR_ACTIVATION_CTRL        : INTEGER := 16#24#;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUMOFOUTNEURONS_DATA_0 =>
                        rdata_data <= RESIZE(int_numOfOutNeurons(15 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    numOfOutNeurons      <= STD_LOGIC_VECTOR(int_numOfOutNeurons);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUMOFOUTNEURONS_DATA_0) then
                    int_numOfOutNeurons(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_numOfOutNeurons(15 downto 0));
                end if;
            end if;
        end if;
    end process;
