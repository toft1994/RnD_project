...
// wrapc file define:
#define AUTOTB_TVIN_output_s "../tv/cdatafile/c.nnlayer.autotvin_output_s.dat"
#define AUTOTB_TVOUT_output_s "../tv/cdatafile/c.nnlayer.autotvout_output_s.dat"
// wrapc file define:
...
// tvout file define:
#define AUTOTB_TVOUT_PC_output_s "../tv/rtldatafile/rtl.nnlayer.autotvout_output_s.dat"
// tvout file define:
...
string get_depth_list () {
  ...
  total_list << "{output_s " << output_s_depth << "}\n";
...
extern "C" void apatb_nnlayer_hw(volatile void * __xlx_apatb_param_input_s, volatile void * __xlx_apatb_param_output_s, volatile void * __xlx_apatb_param_weights_s, volatile void * __xlx_apatb_param_bias_s, short __xlx_apatb_param_numOfInNeurons, short __xlx_apatb_param_numOfOutNeurons, char __xlx_apatb_param_activation) {
...
aesl_fh.read(AUTOTB_TVOUT_PC_output_s, tr.p, tr.tbytes);
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_output_s, 256);
}
#else
{
      static ifstream rtl_tv_out_file;
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_output_s);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > output_s_pc_buffer(256);
          int i = 0;
          bool has_unknown_value = false;
          rtl_tv_out_file >> AESL_token; //data
          while (AESL_token != "[[/transaction]]"){

            has_unknown_value |= RTLOutputCheckAndReplacement(AESL_token, "output_s");
  
            // push token into output port buffer
            if (AESL_token != "") {
              output_s_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (has_unknown_value) {
            cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'x' or 'X' on port " 
                 << "output_s" << ", possible cause: There are uninitialized variables in the C design."
                 << endl; 
          }
  
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 256; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_output_s)[j*2+0] = output_s_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_output_s)[j*2+1] = output_s_pc_buffer[i].range(15, 8).to_int64();
}}}
        } // end transaction
      } // end file is good
...
#endif
unsigned __xlx_offset_byte_param_output_s = 0;
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_output_s, 'b');
transaction<16> tr(256);
  __xlx_offset_byte_param_output_s = 0*2;
  if (__xlx_apatb_param_output_s) {
tr.import<2>((char*)__xlx_apatb_param_output_s, 256, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_output_s, tr.p, tr.tbytes);
}

  tcl_file.set_num(256, &tcl_file.output_s_depth);
#else
// print output_s Transactions
{
aesl_fh.write(AUTOTB_TVIN_output_s, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_output_s = 0*2;
  if (__xlx_apatb_param_output_s) {
    for (int j = 0  - 0, e = 256 - 0; j != e; ++j) {
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_output_s)[j];
aesl_fh.write(AUTOTB_TVIN_output_s, __xlx_tmp_lv.to_string(SC_HEX)+string("\n"));
    }
  }
}

  tcl_file.set_num(256, &tcl_file.output_s_depth);
aesl_fh.write(AUTOTB_TVIN_output_s, end_str());
}
...
nnlayer_hw_stub_wrapper(__xlx_apatb_param_input_s, __xlx_apatb_param_output_s, __xlx_apatb_param_weights_s, __xlx_apatb_param_bias_s, __xlx_apatb_param_numOfInNeurons, __xlx_apatb_param_numOfOutNeurons, __xlx_apatb_param_activation);
CodeState = DUMP_OUTPUTS;
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVOUT_output_s, 'b');
transaction<16> tr(256);
  __xlx_offset_byte_param_output_s = 0*2;
  if (__xlx_apatb_param_output_s) {
tr.import<2>((char*)__xlx_apatb_param_output_s, 256, 0);
...
tr.reorder();
aesl_fh.write(AUTOTB_TVOUT_output_s, tr.p, tr.tbytes);
}
...
// print output_s Transactions
{
aesl_fh.write(AUTOTB_TVOUT_output_s, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_output_s = 0*2;
  if (__xlx_apatb_param_output_s) {
    for (int j = 0  - 0, e = 256 - 0; j != e; ++j) {
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_output_s)[j];
aesl_fh.write(AUTOTB_TVOUT_output_s, __xlx_tmp_lv.to_string(SC_HEX)+string("\n"));
    }
  }
}

  tcl_file.set_num(256, &tcl_file.output_s_depth);
aesl_fh.write(AUTOTB_TVOUT_output_s, end_str());
}
