...
end entity nnlayer_nnIn4_s_axi;

-- ------------------------Address Info-------------------
-- 0x400 ~
-- 0x7ff : Memory 'bias' (256 * 32b)
--         Word n : bit [31:0] - bias[n]
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of nnlayer_nnIn4_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_BIAS_BASE : INTEGER := 16#400#;
    constant ADDR_BIAS_HIGH : INTEGER := 16#7ff#;
    ...
    -- memory signals
    signal int_bias_address0   : UNSIGNED(7 downto 0);
    signal int_bias_ce0        : STD_LOGIC;
    signal int_bias_q0         : UNSIGNED(31 downto 0);
    signal int_bias_address1   : UNSIGNED(7 downto 0);
    ...
    component nnlayer_nnIn4_s_axi_ram is
        generic (
    ...
    end component nnlayer_nnIn4_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_bias
int_bias : nnlayer_nnIn4_s_axi_ram
generic map (
     MEM_STYLE => "block",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_bias_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    ...
    int_bias_address1    <= raddr(9 downto 2) when ar_hs = '1' else waddr(9 downto 2);
...
entity nnlayer_nnIn4_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity nnlayer_nnIn4_s_axi_ram;

architecture behave of nnlayer_nnIn4_s_axi_ram is
