...
architecture behave of runLayer_control_s_axi is
    ...
    constant ADDR_NUMOFINNEURONS_DATA_0 : INTEGER := 16#020#;
    constant ADDR_NUMOFINNEURONS_CTRL   : INTEGER := 16#024#;
    constant ADDR_OUTPUT_R_BASE         : INTEGER := 16#010#;
    constant ADDR_OUTPUT_R_HIGH         : INTEGER := 16#017#;
    constant ADDR_BIAS_BASE             : INTEGER := 16#018#;
    constant ADDR_BIAS_HIGH             : INTEGER := 16#01f#;
    constant ADDR_INPUT_R_BASE          : INTEGER := 16#100#;
    constant ADDR_INPUT_R_HIGH          : INTEGER := 16#1ff#;
    constant ADDR_WEIGHTS_BASE          : INTEGER := 16#400#;
    constant ADDR_WEIGHTS_HIGH          : INTEGER := 16#7ff#;
    ...
    signal int_bias_address0   : UNSIGNED(0 downto 0);
    signal int_bias_ce0        : STD_LOGIC;
    signal int_bias_q0         : UNSIGNED(31 downto 0);
    signal int_bias_address1   : UNSIGNED(0 downto 0);
    ...
    signal int_weights_address0 : UNSIGNED(7 downto 0);
    signal int_weights_ce0     : STD_LOGIC;
    signal int_weights_q0      : UNSIGNED(31 downto 0);
    signal int_weights_address1 : UNSIGNED(7 downto 0);
...
port map (
     ...
     q1        => int_output_r_q1);
-- int_bias
int_bias : runLayer_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
port map (
     ...
     q1        => int_input_r_q1);
-- int_weights
int_weights : runLayer_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_output_r_read = '0') and (int_bias_read = '0') and (int_input_r_read = '0') and (int_weights_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    -- output_r
    int_output_r_address0 <= SHIFT_RIGHT(UNSIGNED(output_r_address0), 1)(0 downto 0);
    ...
    -- bias
    int_bias_address0    <= SHIFT_RIGHT(UNSIGNED(bias_address0), 1)(0 downto 0);
    int_bias_ce0         <= bias_ce0;
    bias_q0              <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_bias_q0, TO_INTEGER(int_bias_shift0) * 16)(15 downto 0));
    int_bias_address1    <= raddr(2 downto 2) when ar_hs = '1' else waddr(2 downto 2);
    ...
    -- weights
    int_weights_address0 <= SHIFT_RIGHT(UNSIGNED(weights_address0), 1)(7 downto 0);
    ...
    int_weights_address1 <= raddr(9 downto 2) when ar_hs = '1' else waddr(9 downto 2);
