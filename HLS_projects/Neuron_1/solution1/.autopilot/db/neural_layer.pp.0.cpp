...
__attribute__((sdx_kernel("nnlayer", 0))) void nnlayer(short int input[256], short int output[256], short int weights[256*256], short int bias[256], unsigned short int numOfInNeurons, unsigned short numOfOutNeurons, unsigned char activation);
...
#pragma HLS inline
 VITIS_LOOP_10_1: for (unsigned char i = 0; i < numOfNeurons; i++)
 {
#pragma HLS PIPELINE
 if (data[i] < 0)
  {
...
__attribute__((sdx_kernel("nnlayer", 0))) void nnlayer(short int input_[256], short int output[256], short int weights_[256*256], short int bias_[256], unsigned short int numOfInNeurons, unsigned short numOfOutNeurons, unsigned char activation) {_ssdm_SpecArrayDimSize(input_, 256);_ssdm_SpecArrayDimSize(output, 256);_ssdm_SpecArrayDimSize(weights_, 65536);_ssdm_SpecArrayDimSize(bias_, 256);
#pragma HLSDIRECTIVE TOP name=nnlayer
# 26 "Neuron_1/neural_layer.cpp"
...
#pragma HLS INTERFACE mode=s_axilite port=return
 ...
 VITIS_LOOP_40_1: for (outNeurons = 0; outNeurons < numOfOutNeurons; outNeurons++) {
  output[outNeurons] = bias[outNeurons];
 }

 VITIS_LOOP_44_2: for (outNeurons = 0; outNeurons < numOfOutNeurons; outNeurons++)
 {
  VITIS_LOOP_46_3: for (short int inNeurons = 0; inNeurons < numOfInNeurons; inNeurons++)
  {
#pragma HLS PIPELINE
