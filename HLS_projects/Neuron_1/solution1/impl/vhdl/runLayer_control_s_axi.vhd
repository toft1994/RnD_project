...
architecture behave of runLayer_control_s_axi is
    ...
    constant ADDR_OUTPUT_R_I_DATA_0     : INTEGER := 16#010#;
    constant ADDR_OUTPUT_R_I_CTRL       : INTEGER := 16#014#;
    constant ADDR_OUTPUT_R_O_DATA_0     : INTEGER := 16#018#;
    constant ADDR_OUTPUT_R_O_CTRL       : INTEGER := 16#01c#;
    constant ADDR_BIAS_DATA_0           : INTEGER := 16#020#;
    constant ADDR_BIAS_CTRL             : INTEGER := 16#024#;
    constant ADDR_NUMOFINNEURONS_DATA_0 : INTEGER := 16#028#;
    constant ADDR_NUMOFINNEURONS_CTRL   : INTEGER := 16#02c#;
    constant ADDR_WEIGHTS_DATA_0        : INTEGER := 16#030#;
    constant ADDR_WEIGHTS_CTRL          : INTEGER := 16#034#;
    constant ADDR_INPUT_R_BASE          : INTEGER := 16#200#;
    constant ADDR_INPUT_R_HIGH          : INTEGER := 16#3ff#;
    ...
    -- internal registers
    signal int_output_r_i      : UNSIGNED(31 downto 0) := (others => '0');
    signal int_output_r_o_ap_vld : STD_LOGIC;
    signal int_output_r_o      : UNSIGNED(31 downto 0) := (others => '0');
...
-- ----------------------- Instantiation------------------
-- int_input_r
int_input_r : runLayer_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_OUTPUT_R_I_DATA_0 =>
                        rdata_data <= RESIZE(int_output_r_i(31 downto 0), 32);
                    when ADDR_OUTPUT_R_O_DATA_0 =>
                        rdata_data <= RESIZE(int_output_r_o(31 downto 0), 32);
                    when ADDR_OUTPUT_R_O_CTRL =>
                        rdata_data(0) <= int_output_r_o_ap_vld;
...
-- ----------------------- Register logic ----------------
    output_r_i           <= STD_LOGIC_VECTOR(int_output_r_i);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_OUTPUT_R_I_DATA_0) then
                    int_output_r_i(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_output_r_i(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_output_r_o <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (output_r_o_ap_vld = '1') then
                    int_output_r_o <= UNSIGNED(output_r_o);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_output_r_o_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (output_r_o_ap_vld = '1') then
                    int_output_r_o_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_OUTPUT_R_O_CTRL) then
                    int_output_r_o_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
