...
//------------------------Parameter----------------------
`define TV_IN_input_r "../tv/cdatafile/c.nnlayer.autotvin_input_r.dat"
`define TV_OUT_output_r "../tv/rtldatafile/rtl.nnlayer.autotvout_output_r.dat"
`define TV_IN_bias "../tv/cdatafile/c.nnlayer.autotvin_bias.dat"
`define TV_IN_weights "../tv/cdatafile/c.nnlayer.autotvin_weights.dat"
`define TV_IN_numOfInputNeurons "../tv/cdatafile/c.nnlayer.autotvin_numOfInputNeurons.dat"
`define TV_IN_numOfOutputNeurons "../tv/cdatafile/c.nnlayer.autotvin_numOfOutputNeurons.dat"
`define TV_IN_activation "../tv/cdatafile/c.nnlayer.autotvin_activation.dat"
...
parameter numOfInputNeurons_DEPTH = 1;
reg [31 : 0] numOfInputNeurons_OPERATE_DEPTH = 0;
parameter numOfInputNeurons_c_bitwidth = 16;
...
parameter numOfInputNeurons_data_in_addr = 16;
...
reg [DATA_WIDTH - 1 : 0] mem_numOfInputNeurons [numOfInputNeurons_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_numOfInputNeurons [ (numOfInputNeurons_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * numOfInputNeurons_DEPTH -1 : 0] = '{default : 'hz};
reg numOfInputNeurons_write_data_finish;
...
//write numOfInputNeurons reg
reg [31 : 0] write_numOfInputNeurons_count = 0;
reg write_numOfInputNeurons_run_flag = 0;
reg write_one_numOfInputNeurons_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & input_r_write_data_finish & bias_write_data_finish & weights_write_data_finish & numOfInputNeurons_write_data_finish & numOfOutputNeurons_write_data_finish & activation_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        numOfInputNeurons_write_data_finish <= 0;
        write_numOfInputNeurons_run_flag <= 0; 
        write_numOfInputNeurons_count = 0;
        count_operate_depth_by_bitwidth_and_depth (numOfInputNeurons_c_bitwidth, numOfInputNeurons_DEPTH, numOfInputNeurons_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            numOfInputNeurons_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_numOfInputNeurons_run_flag <= 1; 
            write_numOfInputNeurons_count = 0;
        end
        if (write_one_numOfInputNeurons_data_done === 1) begin
            write_numOfInputNeurons_count = write_numOfInputNeurons_count + 1;
            if (write_numOfInputNeurons_count == numOfInputNeurons_OPERATE_DEPTH) begin
                write_numOfInputNeurons_run_flag <= 0; 
                numOfInputNeurons_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_numOfInputNeurons
    integer write_numOfInputNeurons_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] numOfInputNeurons_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = numOfInputNeurons_c_bitwidth;
    process_num = 4;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_4_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_numOfInputNeurons_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write numOfInputNeurons data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (numOfInputNeurons_c_bitwidth < 32) begin
                        numOfInputNeurons_data_tmp_reg = mem_numOfInputNeurons[write_numOfInputNeurons_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < numOfInputNeurons_c_bitwidth) begin
                                numOfInputNeurons_data_tmp_reg[j] = mem_numOfInputNeurons[write_numOfInputNeurons_count][i*32 + j];
                            end
                            else begin
                                numOfInputNeurons_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_numOfInputNeurons[write_numOfInputNeurons_count * four_byte_num  + i]!==numOfInputNeurons_data_tmp_reg) begin
                    write (numOfInputNeurons_data_in_addr + write_numOfInputNeurons_count * four_byte_num * 4 + i * 4, numOfInputNeurons_data_tmp_reg, write_numOfInputNeurons_resp);
                    image_mem_numOfInputNeurons[write_numOfInputNeurons_count * four_byte_num + i]=numOfInputNeurons_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_numOfInputNeurons_data_done <= 1;
                @(posedge clk);
                write_one_numOfInputNeurons_data_done <= 0;
            end   
            process_4_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_numOfInputNeurons_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [numOfInputNeurons_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (numOfInputNeurons_c_bitwidth , factor);
  fp = $fopen(`TV_IN_numOfInputNeurons ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_numOfInputNeurons); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < numOfInputNeurons_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_numOfInputNeurons [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
          ...
          if (factor == 1) begin
              mem_numOfInputNeurons [i] = token_tmp;
          end
      end 
      if (factor == 4) begin
          if (i%factor != 0) begin
...
task write_binary_numOfInputNeurons;
    input integer fp;
    input reg[64-1:0] in;
    input integer in_bw;
    reg [63:0] tmp_long;
