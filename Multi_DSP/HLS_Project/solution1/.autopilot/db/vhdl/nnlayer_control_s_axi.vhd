...
architecture behave of nnlayer_control_s_axi is
    ...
    constant ADDR_NUMOFINPUTNEURONS_DATA_0  : INTEGER := 16#0010#;
    constant ADDR_NUMOFINPUTNEURONS_CTRL    : INTEGER := 16#0014#;
    ...
    signal int_numOfInputNeurons : UNSIGNED(15 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUMOFINPUTNEURONS_DATA_0 =>
                        rdata_data <= RESIZE(int_numOfInputNeurons(15 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    numOfInputNeurons    <= STD_LOGIC_VECTOR(int_numOfInputNeurons);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUMOFINPUTNEURONS_DATA_0) then
                    int_numOfInputNeurons(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_numOfInputNeurons(15 downto 0));
                end if;
            end if;
        end if;
    end process;
